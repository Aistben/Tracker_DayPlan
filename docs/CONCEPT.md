...
## 8. Десктоп: Windows .exe — ОТЛОЖЕНО

> **Статус: отложено.** Прототип делается как веб-приложение. Обёртка Tauri была
> удалена, чтобы не тащить неработающий код: собрать .exe можно только на
> Windows, а логика в ней не нуждается. Раздел оставлен как план на будущее —
> фронтенд от обёртки не зависит, вернуть можно в любой момент.
